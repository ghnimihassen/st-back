...
rewards.post("/", authMiddleware, adminMiddleware,
  zValidator("json", z.object({
    ...
    quantity_used:     z.number().int().min(0).optional().default(0),
    discount_value:    z.number().min(0).max(100).nullable().optional(),
    free_product_name: z.string().optional().default(""),
    is_special:        z.boolean().optional().default(false),
    special_label:     z.string().optional().default(""),
  })),
  async (c) => {
...
rewards.put("/:id", authMiddleware, adminMiddleware,
  zValidator("json", z.object({
    ...
    quantity:          z.number().int().min(0).nullable().optional(),
    quantity_used:     z.number().int().min(0).optional(),
    discount_value:    z.number().min(0).max(100).nullable().optional(),
    free_product_name: z.string().optional(),
    is_special:        z.boolean().optional(),
    special_label:     z.string().optional(),
  })),
  async (c) => {
